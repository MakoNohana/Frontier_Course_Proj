...
class Momoka : public QMainWindow
{
    Q_OBJECT

    public:
    explicit Momoka(QWidget* parent = nullptr);
    ~Momoka();
    ...
    //void encodeImageToJPEG(const Mat& image, std::vector<uchar>& encodedImage);
    //void decodeImageFromJPEG(const std::vector<uchar>& encodedImage, cv::Mat& decodedImage);
